...
export default class Terrain {
  constructor(seed, genCities, hills = 1, elevation = 0) {
    ...
    this._dimension = 10000 * SCALE_PLANET_WALK;
    this._segment = 250;
    this._vertices = this._segment + 1;
    this._simplex = new SimplexNoise(seed);
    this.cities = this._cityInfo(genCities);
  ...
  _createGeometry() {
    ...
    for (let i = 0; i < vertices; i++) {
      for (let j = 0; j < vertices; j++) {
        ...
        const isCitySpotIndex = this._checkSpotTaken(this.cities, i, j);
        if (isCitySpotIndex !== undefined) {
          geometry.attributes.position.setY(
            idx,
            this.cities[isCitySpotIndex].elevation
          );
  ...
  _createRoads() {
    ...
    const terrainGeometry = this._geometry;
    ...
    if (cities.length > 1) {
      ...
      console.log(centerCity, cities);
      cities.forEach((city, index) => {
        if (index !== 0) {
          const scale = 2 * SCALE_PLANET_WALK;
          const ls = 400; // length segments
          ...
          console.log("road city.position", city.position);
